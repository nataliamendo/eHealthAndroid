...
@SuppressLint("NewApi")
public class ActualizarPassMActivity extends ActionBarActivity {

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_actualizar_pass_m);
		ActionBar bar = getActionBar();
		//bar.setBackgroundDrawable(new ColorDrawable(Color.parseColor("#64E8CE")));
		bar.hide();
		
		Button login = (Button) findViewById(R.id.actualizaPMF);

		login.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				EditText np = (EditText) findViewById(R.id.passwordUP1);
				EditText np2 = (EditText) findViewById(R.id.passwordUP2);
	...
	public class FetchPTask extends AsyncTask<String, Void, Paciente> {
		...
		protected Paciente doInBackground(String... params) {
			...
			try {
				...
				m.setPass(passSha1);
				p.setSalt(salt);
				EHealthAPI.getInstance(ActualizarPassMActivity.this).putPassM(m);
		...
		protected void onPostExecute(Paciente result) {
			...
			//vuelve al login
			Intent i = new Intent(ActualizarPassMActivity.this,
